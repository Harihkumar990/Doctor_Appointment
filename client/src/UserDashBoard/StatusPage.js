...
import { getappointmentlist } from './MethodsUser';
import { useAuthContext } from '../Redux/redux';
import UserDashboard from './User';
const Status = () =>{
    const {dispatch,listappointment,logindetails} = useAuthContext();
    const navigate = useNavigate();
    useEffect(()=>{
        const getdata = async () =>{
            const data = await  getappointmentlist(logindetails.email);
            if(data === "error"){
                return;
            }
              dispatch({
                type:"List_Of_Appointment",
                payload:data.data
              })
        }
        getdata();
    ...
    return(
        <>
            <UserDashboard/>
          {
            ...
            listappointment.length  > 0  ?  listappointment.map(values =>
